...
FILEPATH="$HOME/Videos/capture/$(date +%s).mp4"
mkdir -p "$HOME/Videos/capture"
...
if [ "$screen_count" -gt 1 ]; then
    ...
    selected_screen=$(echo "$screen_list" | rofi -dmenu -theme "$HOME/.config/rofi/config.rasi")
    
    if [ ! -z "$selected_screen" ]; then
        # Deuxième menu rofi pour le type d'enregistrement
        C2=$(echo -e "region\nfull screen" | rofi -dmenu -theme "$HOME/.config/rofi/config.rasi")
